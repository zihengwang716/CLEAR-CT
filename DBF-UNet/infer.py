"""
Inference script for DBF-UNet.

Loads a trained checkpoint and runs inference on a dataset split (by default 'test'),
saving predictions as .npy files. Does NOT compute any metrics — use your own
evaluation script on the saved predictions.

Usage:
    # Default: run on test split, save to ./predictions/
    python infer.py --ckpt checkpoints/best.pth

    # Run on val split instead
    python infer.py --ckpt checkpoints/best.pth --split val

    # Custom output directory
    python infer.py --ckpt checkpoints/best.pth --output_dir /path/to/output

    # Save predictions in original (un-normalized) scale (recommended for eval)
    python infer.py --ckpt checkpoints/best.pth --denormalize

    # Limit to first N samples (for debugging)
    python infer.py --ckpt checkpoints/best.pth --limit 5
"""
import argparse
...
'''
python /ibex/user/wangz0r/CS_300_Final_Project/CS_300/DBF-UNet/infer.py \
--config /ibex/user/wangz0r/CS_300_Final_Project/CS_300/DBF-UNet/config/config_exp2_all.yaml \
--ckpt /ibex/user/wangz0r/CS_300_Final_Project/CS_300/cache/checkpoint/All_comp__noBH__noW__noTV__ds6__mode1_plain__iter100__rec2048__step0p8__pwr25__noWclip__noWstep__noTVinner/best.pth \
--output_dir /ibex/user/wangz0r/CS_300/cache/result/All_comp__noBH__noW__noTV__ds6__mode1_plain__iter100__rec2048__step0p8__pwr25__noWclip__noWstep__noTVinner
'''