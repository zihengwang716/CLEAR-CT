...
def train_one_epoch(model, loader, optimizer, loss_fn, scaler, device, epoch, log_every, writer, global_step):
    ...
    for it, batch in enumerate(loader):
        ...
        if not torch.isfinite(loss):
            ...
            # Optional deeper diagnostics on first occurrence
        ...
        if (it + 1) % log_every == 0:
            ...
            print(f"  [epoch {epoch} iter {it+1}/{len(loader)}] "
                  ...
                  f"ssim_l={loss_dict['ssim_loss']:.4f} "
                  f"fft={loss_dict['fft']:.4f} "
                  f"psnr={p:.2f}dB "
                  f"({ips:.2f} img/s)")
            writer.add_scalar('train/loss', loss.item(), global_step)
            writer.add_scalar('train/l1', loss_dict['l1'], global_step)
            writer.add_scalar('train/ssim_loss', loss_dict['ssim_loss'], global_step)
...
def main():
    ...
    # ============================================================
    # ---- 实例化 Sparse-view 消融模型 (保留频率，移除注意力) ----
    # ============================================================
    model = FreqUNet_SparseView(
        ...
        freq_channels=cfg['model']['freq_channels'], # 此处需要提供频率参数
    ...
    if args.resume and Path(args.resume).exists():
        start_epoch, best_psnr = load_checkpoint(args.resume, model, optimizer, scheduler, map_location=device)
    ...
    for epoch in range(start_epoch, cfg['train']['epochs']):
        ...
        if (epoch + 1) % cfg['log']['val_every'] == 0:
            ...
            if val_metrics['psnr'] > best_psnr:
                ...
                save_checkpoint(Path(cfg['log']['ckpt_dir']) / "best.pth", model, optimizer, scheduler, epoch + 1, best_psnr, cfg)
                print(f"  >>> New best PSNR: {best_psnr:.2f}dB, saved.")

        if (epoch + 1) % cfg['log']['save_every'] == 0:
            save_checkpoint(Path(cfg['log']['ckpt_dir']) / f"epoch_{epoch+1:04d}.pth", model, optimizer, scheduler, epoch + 1, best_psnr, cfg)
