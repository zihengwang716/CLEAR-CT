#!/usr/bin/env python3
"""
L-curve 法选择 λ：AGD+TV 在 sparse-view 下的正则化参数调优。

在训练 slice 上对一组候选 λ 分别跑 AGD+TV 重建，记录数据保真项和 TV 项，
画 L-curve 找拐点 λ，然后在验证 slice 上评估 plain AGD vs AGD+TV(λ*)。

用法:
  python lambda_lcurve.py \
...
def find_lcurve_corner(log_fidelity, log_tv, lambdas):
    """通过离散曲率找 L-curve 拐点。返回最佳 λ 的索引。"""
    n = len(log_fidelity)
    if n < 3:
        return n // 2

    # 离散曲率: κ = (x'y'' - y'x'') / (x'^2 + y'^2)^(3/2)
    curvature = np.zeros(n)
    for i in range(1, n - 1):
        dx1 = log_fidelity[i] - log_fidelity[i - 1]
        dy1 = log_tv[i] - log_tv[i - 1]
        dx2 = log_fidelity[i + 1] - log_fidelity[i]
        dy2 = log_tv[i + 1] - log_tv[i]
        # 二阶差分近似
        xp = (dx1 + dx2) / 2
        yp = (dy1 + dy2) / 2
        xpp = dx2 - dx1
        ...
        denom = (xp ** 2 + yp ** 2) ** 1.5
        if denom > 1e-15:
            curvature[i] = (xp * ypp - yp * xpp) / denom

    # 拐点 = 曲率最大处 (排除端点)
    best_idx = 1 + np.argmax(curvature[1:-1])
    return best_idx
...
def main():
    p = argparse.ArgumentParser(description="L-curve method for lambda (AGD+TV)")
    ...
    # ═══════════════════════════════════════════════════════
    # Phase 1: 在训练 slice 上画 L-curve
    # ═══════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("Phase 1: L-curve on training slices")
    print("=" * 60)

    # 对每个 λ，存储所有 slice 的 (data_fidelity, tv_norm)
    all_fidelity = {lam: [] for lam in args.lambdas}
    all_tv = {lam: [] for lam in args.lambdas}
    all_psnr = {lam: [] for lam in args.lambdas}
    ...
    for sid in args.train_slices:
        ...
        for lam in args.lambdas:
            ...
            fid = recon.compute_data_fidelity(rec, sino)
            tv = tv_norm(rec)
            all_fidelity[lam].append(fid)
            ...
            if gt is not None:
                ...
                psnr_val = m['PSNR']
                all_psnr[lam].append(psnr_val)

            psnr_str = f"PSNR={psnr_val:.2f}" if psnr_val else "no GT"
            print(f"  slice {sid:05d} | λ={lam:<10} | "
                  f"||Ax-y||²={fid:.2e}  TV={tv:.2e}  {psnr_str}  ({dt:.1f}s)")

    # ── 计算每个 λ 的平均 fidelity 和 tv ──
    lambdas_sorted = sorted(args.lambdas)
    avg_fid = [np.mean(all_fidelity[l]) for l in lambdas_sorted]
    avg_tv = [np.mean(all_tv[l]) for l in lambdas_sorted]
    log_fid = np.log10(np.array(avg_fid) + 1e-30)
    log_tv = np.log10(np.array(avg_tv) + 1e-30)

    # ── 找拐点 ──
    corner_idx = find_lcurve_corner(log_fid, log_tv, lambdas_sorted)
    best_lam = lambdas_sorted[corner_idx]

    print(f"\n--- L-curve summary ---")
    print(f"{'λ':>12s}  {'log||Ax-y||²':>14s}  {'logTV':>10s}  {'Avg PSNR':>10s}")
    for i, lam in enumerate(lambdas_sorted):
        psnr_str = f"{np.mean(all_psnr[lam]):.4f}" if all_psnr[lam] else "N/A"
        marker = " <<<" if i == corner_idx else ""
        print(f"  {lam:12.6f}  {log_fid[i]:14.4f}  {log_tv[i]:10.4f}  {psnr_str:>10s}{marker}")
    print(f"\n>>> L-curve corner: λ = {best_lam}")

    # ── 画 L-curve 图 ──
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    ax.plot(log_fid, log_tv, 'bo-', markersize=6)
    ax.plot(log_fid[corner_idx], log_tv[corner_idx], 'r*', markersize=15,
            label=f'corner λ={best_lam}')
    for i, lam in enumerate(lambdas_sorted):
        ax.annotate(f'{lam}', (log_fid[i], log_tv[i]),
                    textcoords="offset points", xytext=(8, 5), fontsize=7)
    ax.set_xlabel('log₁₀ ||Ax - y||²')
    ax.set_ylabel('log₁₀ TV(x)')
    ax.set_title(f'L-curve (downsample ×{args.downsample})')
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig_path = os.path.join(args.out_dir, f"lcurve_ds{args.downsample}.png")
    fig.savefig(fig_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"L-curve 图: {fig_path}")

    # 保存训练结果 CSV
    train_csv = os.path.join(args.out_dir, "lcurve_train.csv")
    with open(train_csv, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(["lambda", "avg_data_fidelity", "avg_tv_norm", "avg_psnr",
                     "log_fidelity", "log_tv", "is_corner"])
        for i, lam in enumerate(lambdas_sorted):
            psnr_avg = np.mean(all_psnr[lam]) if all_psnr[lam] else ""
            w.writerow([lam, avg_fid[i], avg_tv[i], psnr_avg,
                        log_fid[i], log_tv[i],
                        1 if i == corner_idx else 0])
    print(f"训练结果: {train_csv}")

    # ═══════════════════════════════════════════════════════
    # Phase 2: 验证 plain AGD vs AGD+TV(λ_corner)
    # ═══════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    ...
    for sid in args.val_slices:
        ...
        val_rows.append({
            "slice": sid, "method": "plain_AGD", "lambda": 0.0,
            "PSNR": m_plain['PSNR'], "SSIM": m_plain['SSIM'],
            "RMSE": m_plain['RMSE'], "MAE": m_plain['MAE'],
        })
        val_rows.append({
            "slice": sid, "method": "AGD_TV_lcurve", "lambda": best_lam,
            "PSNR": m_tv['PSNR'], "SSIM": m_tv['SSIM'],
            "RMSE": m_tv['RMSE'], "MAE": m_tv['MAE'],
        })

    # ── 平均值 ──
    for method_key in ["plain_AGD", "AGD_TV_lcurve"]:
        rows = [r for r in val_rows if r["method"] == method_key]
        if rows:
            val_rows.append({
                "slice": "AVG", "method": method_key,
    ...
    # ── CSV ──
    val_csv = os.path.join(args.out_dir, f"lcurve_val_ds{args.downsample}.csv")
    ...
    for method_key in ["plain_AGD", "AGD_TV_lcurve"]:
        ...
        if avg_row:
            r = avg_row[0]
            print(f"  {method_key:18s}  λ={r['lambda']:<10}  "
